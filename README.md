# Tic Tac Toe Game
If you have not played Tic Tac Toe and are not familiar with the game, here is a complete description from [Wikipedia](https://en.wikipedia.org/wiki/Tic-tac-toe). This Repository Contains Tic Tac Toe Game project which is suitable for school or college mini project. It is written in c++ language in code::blocks IDE with MinGW compiler. It is console application without graphics. Whole project is completely based on 2D arrays to stores both players input and show results. You learn how to store and reterive the data from 2d arrays and also how to use functions, I also Validate the user input in this game e.g when inputted value is not the required value so program must throws an error and taking input again from user until when the program gets the required value. Following features can be found in this project which is listed below.
 1. Both Player Plays a Game Easily
 2. Awesome Interface of Game on console
 3. Input Validation with throwing errors
 4. Shows Win, Loss and Draw message
 5. Clear Screen on Every user Input
 6. Very Easy to Play


![Repository Banner](tic-tac-toe-game-cplusplus.jpg)


# Introduction to Tic Tac Toe Game Using C++
